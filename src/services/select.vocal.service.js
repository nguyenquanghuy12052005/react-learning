...
class selectVocalService {

    async getAllVoc() {
    const response = await axiosInstance.get('/vocals/');
    return response.data;
  }

  // Lấy bài viết theo ID
  async getVocById(id) {
    const response = await axiosInstance.get(`/vocals/${id}`);
    return response.data;
  }


     // Cập nhật voc
  async updatePost(id, vocData) {
    const response = await axiosInstance.put(`/vocals/${id}`, vocData);
    return response.data;
  }
...
export default new selectVocalService();
